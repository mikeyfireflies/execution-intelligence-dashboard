...
const INITIATIVE_DB_ID = process.env.NOTION_INITIATIVE_TRACKER_DATABASE_ID;
...
// Extract names from various property types as an array of strings
function extractPeopleArray(page, fieldName) {
  const prop = page.properties[fieldName];
  if (!prop) return [];
  ...
  switch (prop.type) {
    ...
    case 'multi_select':
      return prop.multi_select?.map(s => s.name) || [];
    case 'rich_text':
      return prop.rich_text?.map(t => t.plain_text).join('').split(',').map(s => s.trim()).filter(Boolean) || [];
    case 'title':
      return prop.title?.map(t => t.plain_text).join('').split(',').map(s => s.trim()).filter(Boolean) || [];
    default:
      return [];
  }
}
...
function normalizeGoal(page) {
  const goalTitle = extractProperty(page, FIELD_MAP.goalTitle) || 'Untitled';
  const parentGoal = extractProperty(page, FIELD_MAP.parentGoal);

  // Owner: always a single person (first in the people list)
  const ownerPeople = extractPeopleArray(page, FIELD_MAP.owner);
  const owner = ownerPeople[0] || 'Unassigned';
  const ownerOverflow = ownerPeople.slice(1);

  // Contributors: separate Notion people field + any overflow from owner field
  const explicitContributors = extractPeopleArray(page, FIELD_MAP.contributors);
  const contributors = [...new Set([...ownerOverflow, ...explicitContributors])];

  const squad = extractProperty(page, FIELD_MAP.squad);
  const status = extractProperty(page, FIELD_MAP.status) || 'Unknown';
  const priority = extractProperty(page, FIELD_MAP.priority) || 'Medium';
  const dueDate = extractProperty(page, FIELD_MAP.dueDate);
  const effortPoints = extractProperty(page, FIELD_MAP.effortPoints) || 0;
  const sourceUrl = extractProperty(page, FIELD_MAP.sourceUrl);
  const lastUpdated = extractProperty(page, FIELD_MAP.lastUpdated) || page.last_edited_time;
  const department = extractProperty(page, FIELD_MAP.department) || 'Unassigned';
  const initiativeType = extractProperty(page, FIELD_MAP.initiativeType) || 'Unclassified';
  const notes = extractProperty(page, FIELD_MAP.notes) || '';

  const now = new Date();
  const targetDateObj = dueDate ? new Date(dueDate) : null;
  const sevenDaysAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);

  const isSlipped = targetDateObj && targetDateObj < now && status !== 'Complete';
  const slippageDays = isSlipped ? Math.floor((now - targetDateObj) / 86400000) : 0;

  let dataCompleteness = 'Missing';
  if (owner !== 'Unassigned' && dueDate && status !== 'Unknown') {
    dataCompleteness = 'Complete';
  } else if (owner !== 'Unassigned' || dueDate || status !== 'Unknown') {
    dataCompleteness = 'Partial';
  }

  const lastEditedObj = lastUpdated ? new Date(lastUpdated) : null;
  const isStale = lastEditedObj && lastEditedObj < sevenDaysAgo && status !== 'Complete';
