...
export function takeSnapshot(companyMetrics, individualMetrics, squadMetrics) {
    ...
    const snapshot = {
        ...
        owners: Object.entries(individualMetrics).reduce((acc, [name, m]) => {
            acc[name] = {
                ...
                effortInProgress: m.effortInProgress,
                riskLevel: m.riskLevel,
            };
            return acc;
        }, {}),
        squads: Object.entries(squadMetrics).reduce((acc, [name, m]) => {
            acc[name] = {
                completed: m.completed,
                active: m.active,
                overdue: m.overdue,
