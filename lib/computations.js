...
function isUnknown(status) {
    return !status || status.trim() === '' || status.toLowerCase() === 'unknown';
}
...
export function computeIndividualMetrics(goals) {
    ...
    goals.forEach(goal => {
        ...
        if (isUnknown(goal.status)) o.unknown++;
    ...
    // Second pass: process contributors
    const ensureOwnerEntry = (name) => {
        if (!owners[name]) {
            owners[name] = {
                name,
                goals: [],
                contributedGoals: [],
                collaborators: {},
                active: 0, blocked: 0, overdue: 0, completed: 0,
                notStarted: 0, unknown: 0, highPriority: 0, totalEffort: 0,
                effortInProgress: 0, daysSinceLastUpdate: 999,
                risks: [], statusBreakdown: {}, velocity: [],
                riskLevel: 'green', totalGoals: 0,
            };
            for (let i = 3; i >= 0; i--) {
                const weekStart = startOfWeek(subDays(NOW(), i * 7), { weekStartsOn: 1 });
                owners[name].velocity.push({ week: format(weekStart, 'MMM dd'), points: 0 });
            }
        }
    };

    goals.forEach(goal => {
        const ownerName = goal.owner || 'Unassigned';
        const contributors = goal.contributors || [];

        contributors.forEach(contrib => {
            ensureOwnerEntry(contrib);
            // Track this goal as a contributed goal for the contributor
            owners[contrib].contributedGoals.push(goal);
            // Build collaborator pairs: contributor ↔ owner
            owners[contrib].collaborators[ownerName] = (owners[contrib].collaborators[ownerName] || 0) + 1;
            // Also build the reverse: owner ↔ contributor
            if (owners[ownerName]) {
                owners[ownerName].collaborators[contrib] = (owners[ownerName].collaborators[contrib] || 0) + 1;
            }
        });
    });
...
export function computeSquadMetrics(goals) {
    ...
    goals.forEach(goal => {
        ...
        if (isUnknown(goal.status)) s.unknown++;
...
export { isActive, isBlocked, isCompleted, isNotStarted, isUnknown, isOverdue, daysSinceUpdate, isHighPriority, detectRisks, riskLevel };
